...
public class Mp3FileExtended extends FileWrapper {
	...
	protected Mp3FileExtended() {
	}

	public Mp3FileExtended(File file) throws IOException, UnsupportedTagException, InvalidDataException {
		this(file, DEFAULT_BUFFER_LENGTH, true);
	}
	
	public Mp3FileExtended(File file, int bufferLength, boolean scanFile) throws IOException, UnsupportedTagException, InvalidDataException {
		super(file);
		...
		try {
			...
			if (bytesRead == HEADER_LENGTH) {
				try {
					...
					return HEADER_LENGTH + BufferTools.unpackSynchsafeInteger(bytes[DATA_LENGTH_OFFSET], bytes[DATA_LENGTH_OFFSET + 1], bytes[DATA_LENGTH_OFFSET + 2], bytes[DATA_LENGTH_OFFSET + 3]);
	...
	public void save() throws IOException, NotSupportedException {
		RandomAccessFile saveFile = new RandomAccessFile(file.getAbsolutePath(), "rw");
		try {
			if (hasId3v2Tag()) {
